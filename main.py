...
class Game():
    def __init__(self):
        ...
        vectorlist = [[50, 50], [-50, 50], [50, -100]]
        self.vectorspeedlist = [1, -1, 1]
        self.vector = Vector((50, 50))
        self.base_vector = self.vector
        for cords in vectorlist:
            self.vector = Vector(cords, parentVector=self.vector)
    ...
    def update(self):
        ...
        while type(curr_vector.parentVector) != numpy.ndarray:
            curr_vector.set_rad(curr_vector.radian + self.dt/1000*2*pi*self.vectorspeedlist[i])
            curr_vector.update()
